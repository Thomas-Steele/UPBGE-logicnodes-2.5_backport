from .animationsocket import NodeSocketLogicAnimation
from .armaturesocket import NodeSocketLogicArmature
from .axissignedsocket import NodeSocketLogicAxisSigned
from .axissocket import NodeSocketLogicAxis
from .bitmasksocket import NodeSocketLogicBitMask
from .blendmodesocket import NodeSocketLogicBlendMode
from .boneconstraintsocket import NodeSocketLogicBoneConstraint
from .bonesocket import NodeSocketLogicBone
from .boolcondition import NodeSocketLogicBoolCondition  # deprecated
from .booleansocket import NodeSocketLogicBoolean
from .camerasocket import NodeSocketLogicCamera
#from .collectionsocket import NodeSocketLogicCollection ### Logic Nodes 2.8+ Implementation
from .colorrgbasocket import NodeSocketLogicColorRGBA
from .colorrgbsocket import NodeSocketLogicColorRGB
from .conditionsocket import NodeSocketLogicCondition
from .curvesocket import NodeSocketLogicCurve
from .dictsocket import NodeSocketLogicDictionary
from .filepathsocket import NodeSocketLogicFilePath
from .floatanglesocket import NodeSocketLogicFloatAngle
from .floatfactorsocket import NodeSocketLogicFloatFactor
from .floatpositivesocket import NodeSocketLogicFloatPositive
from .floatsocket import NodeSocketLogicFloat
from .fontsocket import NodeSocketLogicFont
from .gamepropertysocket import NodeSocketLogicGameProperty
#from .geometrynodetreesocket import NodeSocketLogicGeometryNodeTree ### Logic Nodes 2.8+ Implementation
from .globalcategorysocket import NodeSocketLogicGlobalCategory
from .globalpropertysocket import NodeSocketLogicGlobalProperty
from .imagesocket import NodeSocketLogicImage
from .integerpositivecentsocket import NodeSocketLogicIntegerPositiveCent
from .integerpositivesocket import NodeSocketLogicIntegerPositive
from .integersocket import NodeSocketLogicInteger
from .invertxysocket import NodeSocketLogicInvertXY
from .keyboardkeysocket import NodeSocketLogicKeyboardKey
from .lightsocket import NodeSocketLogicLight
from .listitemsocket import *
from .listsocket import NodeSocketLogicList
from .logicbricksocket import NodeSocketLogicBrick
from .logictreesocket import NodeSocketLogicTree
from .loopcountsocket import NodeSocketLogicLoopCount
from .materialnodesocket import NodeSocketLogicTreeNode
from .materialsocket import NodeSocketLogicMaterial
from .matrixsocket import NodeSocketLogicMatrix
from .meshsocket import NodeSocketLogicMesh
from .mousebuttonsocket import NodeSocketLogicMouseButton
from .navmeshsocket import NodeSocketLogicNavMesh
from .nodegroupnodesocket import NodeSocketLogicNodeGroupNode
from .nodegroupsocket import NodeSocketLogicNodeGroup
from .objectnamesocket import NodeSocketLogicObjectName  # deprecated
from .objectsocket import NodeSocketLogicObject
from .parametersocket import NodeSocketLogicParameter
from .playmodesocket import NodeSocketLogicPlayMode
from .pythonsocket import NodeSocketLogicPython
from .scenesocket import NodeSocketLogicScene
from .soundfilesocket import NodeSocketLogicSoundFile
from .speakersocket import NodeSocketLogicSpeaker
from .stringsocket import NodeSocketLogicString, NLQuotedStringFieldSocket
from .textsocket import NodeSocketLogicText
from .timesocket import NodeSocketLogicTime
from .uisocket import NodeSocketLogicUI
from .valueoptionalsocket import NodeSocketLogicValueOptional
from .valuesocket import NodeSocketLogicValue
from .vectorsocket import NodeSocketLogicVector
from .vectorxyanglesocket import NodeSocketLogicVectorXYAngle
from .vectorxysocket import NodeSocketLogicVectorXY
from .vectorxyzanglesocket import NodeSocketLogicVectorXYZAngle
from .vectorxyzsocket import NodeSocketLogicVectorXYZ
from .vectorxyzvelocitysocket import NodeSocketLogicVectorXYZVelocity
from .xyzsocket import NodeSocketLogicXYZ
from .socket import NodeSocketLogicVirtual
from .logictreepropertysocket import NodeSocketLogicTreeProperty
from .constraintsocket import NodeSocketLogicConstraint

from .deprecated import *


# SetVSync: REPLACE
# Gamepad Look: REPLACE
# Check Distance: REPLACE (Compare Distance)
# Mouse Wheel node: -mode, +difference / REPLACE
# Set Material: REPLACE

# Typecast: REPLACE
# Add Constraint: REPLACE

# Cursor Behavior: REMOVED
